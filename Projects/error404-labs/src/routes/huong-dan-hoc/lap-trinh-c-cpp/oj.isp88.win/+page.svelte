<script lang="ts">
	...
	const onLoadLessonV2 = async (fileName?: string) => {
		...
		const module = await import(`$lib/oj.isp88.win/${fileName}.svelte`);
		...
		localStorage.setItem('oj.isp88.win', JSON.stringify(currentLesson));

		scrollToTop();
	};

	onMount(() => {
		if (localStorage.getItem('oj.isp88.win')) {
			const currentLesson: LESSON = JSON.parse(
				JSON.parse(JSON.stringify(localStorage.getItem('oj.isp88.win')))
			);
